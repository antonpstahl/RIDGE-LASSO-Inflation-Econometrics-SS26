...
│   ├── reporting.py             Figure generation (fig_01 to fig_13) + table export
...
**Central finding:** Lag model (ADL, own lags only) RMSE/RW = 1.12 - LASSO+HICP (own lags + macro) RMSE/RW = 1.57, so macro value-added beyond persistence ≈ 0 (ceteris paribus).
...
**Sample-extension robustness (AP32):** Dropping the single binding series (`BS_Produktionserwart`, ends 2024-09) extends the OOS window to **2025-12** (+14 months, post-shock segment 2023-04-2025-12, n=28, was 14). In the calmer post-shock regime **AR, LASSO+HVPI** beat the RW significantly (DM/CW p<0.10, best LASSO+HVPI, RMSE/RW=0.95). This is the first time the *RW-unbeatable* claim is tested out-of-sample outside the energy price shock. Table: `results/robustness_extended.csv`.
<!-- RESULTS:END -->
...
2. **No model beats the naive Random Walk.** Across all horizons (h ∈ {1,3,6,12}),
   `ŷ_t = y_{t-1}` is the hardest benchmark, and the macroeconomic models remain above it.
...
test (HLN correction, T=36) confirms that no model significantly beats the RW at the 5% level.
